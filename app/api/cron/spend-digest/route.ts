...
// every tenant + admin/internal calls. Companion to the existing
// spend-ceiling cron (which pauses individual tenants when they cross
// their cap) — this one is about giving the founder visibility into the
// shape of the spend curve over time.
//
// Reads .openclaw/operator/spend.jsonl (same source spend-ceiling uses).
// Groups by tenant + provider + spend kind, prints a markdown-ish table
// in the email body. No external state.
//
...
function readRecentSpend(): SpendEntry[] {
  const fp = spendPath();
...
<div style="font-family:-apple-system,sans-serif;color:#0F0E0C;max-width:680px;margin:0 auto;padding:32px">
  ...
  <p style="color:#888;font-size:12px;margin-top:32px">
    Source: <code>.openclaw/operator/spend.jsonl</code> · generated by the Operator spend-digest cron
  </p>
</div>`;
